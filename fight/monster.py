...
class Monster:
	...
	def attack_player(self, phenny, cur_round, attack, player, first_turn = False):
		...
		if attack.damage_base > 0:
			...
			# Critical hit
			if attack.is_critical_hit(self.stats['strength'], player.level):
			...
			modifier = attack.get_attribute_modifier(self, player)
			...
			if modifier > 2:
				...
				phenny.say("%s It was extremely ineffective." % (self.announce_prepend(), player.site_username))
			# Damage randomizer
