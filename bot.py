...
class Phenny(irc.Bot): 
   def __init__(self, config): 
      args = (config.nick, config.name, config.channels, config.advchan, config.password)
   ...
   def input(self, origin, text, bytes, match, event, args): 
      class CommandInput(unicode): 
         def __new__(cls, text, origin, bytes, match, event, args): 
            ...
            s.args = args
            s.admin = origin.nick.lower() in (admin.lower() for admin in self.config.admins)
            s.mod = origin.nick.lower() in (mod.lower() for mod in self.config.mods)
            s.owner = origin.nick.lower() == self.config.owner.lower()
            return s
   ...
   def callGazelleApi(self, input):
      getdata =  {"apikey": self.config.gazelle_apikey}
      getdata = dict(getdata.items() + input.items())
      try:
         resp = requests.get(self.config.gazelle_apiurl, params=getdata)
         return json.loads(resp.text)
      except:
         return False
      #data = urllib2.urlopen(apiUrl).read().decode('utf-8').replace("'", "\"")
